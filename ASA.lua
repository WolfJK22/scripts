...
themeitems = {u8'Синяя', u8'Красная', u8'Коричневая', u8'Аква', u8'Чёрная', u8'Фиолетовая', u8'Черно-оранжевая', u8'Светло-темная', u8'Серая', u8'Вишневая', u8'Светло-красная', u8'Темно-красная', u8'Монохром', u8'Ярко-синяя', u8'Белая', u8'Салатовая', u8'Темно-зеленая', u8'Темно-синяя'}
styleitems = {u8'Строгий', u8'Мягкий'}

arr_rang = {u8'Стажер', u8'Инструктор', u8'Ст. Инструктор', u8'Агент страхования', u8'Инспектор', u8'Управляющий', u8'Зам. Директора', u8'Директор'}
...
local Toggle = {
    ...
    IDcard = imgui.ImBool(config.HF041120.IDcard)
}
local Radio = {
...
local Input = {
    ...
    My_rang = imgui.ImBuffer(''..config.Settings.ASRang, 64),
    Pass = imgui.ImBuffer(''..config.HF041120.Pass, 64)
}
local Krit = {
...
local script_vers = 14.2
local script_vers_text = '1.14.2'

local update_url = 'https://raw.githubusercontent.com/WolfJK22/scripts/main/ASAupdate.ini'
local update_path = getWorkingDirectory() .. '/ASA config/ASAupdate.ini'

local script_url = 'https://github.com/WolfJK22/scripts/blob/main/ASA.luac?raw=true'
...
---- Variables after SAMPon ----
	...
	nrpnick = sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))
...
for i = 1, #Notepad do
 ...
 -- Dincamic variables --
    ...
    nrpnick1 = sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)))
    if Radio['Gender'].v == 1 then Sex = '' SexL = '' else Sex = 'а' SexL = 'ла' end
    if Toggle['IDcard'].v then Pass_or_id, IDa, IDe, IDy, PassE = 'ID-карт', 'а', 'е', 'у', '' else Pass_or_id, IDa, IDe, IDy, PassE = 'паспорт', '', '', '', 'е' end
    date = os.date("%d.%m.%Y")
    timehm = os.date('%H:%M')
-- Dincamic variables --
...
if RunCarExam then
    ...
    local bx, by, bz = -2087.5845, -182.4629, 35.3203 -- 1
    local ex, ey, ez = -2088.2183, -267.8858, 35.3203 -- 2
    local qx, qy, qz = -2081.6741, -208.6142, 35.3203 -- 3
    local wx, wy, wz = -2056.8645, -218.1759, 35.3274 -- 4
    local ux, uy, uz = -2052.0320, -226.5643, 35.3203 -- 5
    local px, py, pz = -2087.5076, -188.8506, 35.3203 -- 6
    if IsPlayerInRangeOfPoint(bx, by, bz, 4) and NextStepCar == 1 then 
    wait(500)
        sampSendChat("Начинаем движение, преодолев последовательно 2 эстакады.")
    ...
    wait(500)
        sampSendChat("Развернитесь на 180 градусов, после чего проедьте конусы змейкой.")
    ...
    wait(500)
        sampSendChat("Теперь поворачиваем направо.")
    ...
    wait(500)
        sampSendChat("Выполните параллельную парковку между двумя прицепами.")
    ...
    wait(500)
        sampSendChat("После чего развернитесь и продолжайте движение к выезду.")
        if Toggle['Checkpoints_st'].v then setMarker(2, ox, oy, oz, 3, 0xFF0000FF) end
        NextStepCar = 6
    elseif IsPlayerInRangeOfPoint(px, py, pz, 3) and NextStepCar == 6 then
    wait(500)
    sampSendChat("Теперь паркуем автомобиль туда, откуда его взяли и глушим двигатель.")
    NextStepCar = 7
    end
end
...
if RunMotoExam then
    local ax, ay, az = getCharCoordinates(1)
    local bx, by, bz = -2087.2485, -184.5701, 35.3203 -- 1
    local ex, ey, ez = -2081.6045, -260.9614, 35.3203 -- 2
    local vx, vy, vz = -2015.4613, -186.6989, 35.3203 -- 3
    if IsPlayerInRangeOfPoint(bx, by, bz, 3) and NextStepMoto == 1 then
    wait(500)
        sampSendChat('Начинаем движение вперед, объезжая конусы змейкой.')
    ...
    wait(500)
        sampSendChat('Поверните налево, продолжайте движение по направлению полосы к её выходу.')
        if Toggle['Checkpoints_st'].v then setMarker(2, qx, qy, qz, 3, 0xFF0000FF) end
        NextStepMoto = 3
    elseif IsPlayerInRangeOfPoint(vx, vy, vz, 3) and NextStepMoto == 3 then
    wait(500)
...
if RunBigCarExam then
    local ax, ay, az = getCharCoordinates(1)  
    local bx, by, bz = -2047.5356, -82.2703, 35.1641 -- 1
...
function ev.onServerMessage(color, text)
    ...
    if string.find(text, '.+_.+ повысил Вас до ранга: "(.+)"')  then --and color == 647747839
        local rang_up = string.match(text, '.+_.+ повысил Вас до ранга: "(.+)"')
        Input['My_rang'].v = u8(rang_up)
        config.Settings.ASRang = u8(rang_up) saveJson()
    end
--[[
...
function ev.onShowDialog(id, style, title, button1, button2, text)
    if string.find(title, 'Статистика') and CheckStats then
        local rang = string.match(text, 'Работа/Должность: 		{0099ff}(.+)\n{FFFFFF}Ранг')
        Input['My_rang'].v = u8(rang)
        config.Settings.ASRang = u8(rang) saveJson()
        CheckStats = false
        lua_thread.create(function() wait(0) sampCloseCurrentDialogWithButton(1) end)
        --]]
    end
end
...
function HelloFunc()
    if not tonumber(Input['My_rang'].v) and Input['My_rang'].v ~= '' then
    sampSendChat('Здравствуйте, я '..u8:decode(Input['My_rang'].v)..', '..rpnick..', могу чем-то помочь?') else
        sampAddChatMessage(tag_text..' Впишите свой ранг в настройках.', tag_clr)
        sampSendChat('Здравствуйте, я '..rpnick..', могу чем-то помочь?') end
end
        ...
        imgui.Begin('License Center Assistant ##1', Window['Main'], imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.MenuBar + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
            ...
            if Child['InterviewM'] then
                ...
                if imgui.Button(u8'Приветствие [1]', imgui.ImVec2(Xbi, 30)) then
                    lua_thread.create(function()
                        ...
                        if isKeyJustPressed(0x31) then
                            sampSendChat('Хорошо, покажите Ваш'..IDy..' '..Pass_or_id..IDy..', лицензии и медицинскую карту.')
                            wait(2200)
                            sampSendChat('/n Показать '..Pass_or_id..IDy..' - /pass '..myid..' , мед. карту - /med '..myid..', лицензии - /lic '..myid..'.')
                ...
                if imgui.Button(u8'НонРП ник', imgui.ImVec2(Xbi2, Ybi2)) then
                    lua_thread.create(function()
                        sampSendChat('У вас опечатка в '..Pass_or_id..IDe..PassE..', вы не приняты.')
                        wait(2200)
            ...
            elseif Child['SettingsMain'] then
                ...
                local XposEl = 161
                    imgui.TextRGB('Основные настройки', 2) imgui.Separator() imgui.SetCursorPosY(28)
                    imgui.Text(u8'Ваш ранг:') imgui.SameLine() imgui.PushItemWidth(100) imgui.SetCursorPosY(27)
                    if imgui.InputText('##ASRang', Input['My_rang']) then config.Settings.ASRang = Input['My_rang'].v saveJson() end
                    imgui.SameLine() imgui.SetCursorPosX(168) if imgui.ButtonHint(u8'Вписать ранг из паспорта', fa.ICON_SEARCH, 0.2) then CheckStats = true sampSendChat('/stats') end
                    imgui.Text(u8'Ваш пол:') imgui.SameLine() imgui.SetCursorPosX(85)
                    if imgui.RadioButton(u8'Муж', Radio['Gender'], 1) then config.Settings.Gender = Radio['Gender'].v saveJson() end imgui.SameLine()
                    ...
                    imgui.Text(u8'ID-карта:') imgui.SameLine() 
                    imgui.TextQuestion(u8'Для серверов где не паспорт, а ID-карта') imgui.SameLine() imgui.SetCursorPosX(XposEl)
                    if imgui.ToggleButton('   Id Card', Toggle['IDcard']) then config.HF041120.IDcard = Toggle['IDcard'].v saveJson() end
            ...
            elseif Child['Support'] then
				...
				imgui.TextRGB('Внес правки - {e2335c}Aurelio Noviello', 2)
                imgui.Text('') imgui.Text('')
                imgui.TextRGB('Сервер {e2335c}Ruby', 2)
        ...
        imgui.Begin(u8'Действия с '..targNick..'['..tid..']##2', Window['Target'], imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
            ...
            if imgui.Button(u8'Сотрудник', imgui.ImVec2(Xbm, Ybm)) then Child['Work'] = not Child['Work'] Child['Exam'] = false end
                ...
                if Child['Exam'] then
                    ...
                    if imgui.Button(u8'Лицензия на оружие', imgui.ImVec2(Xsb-40,Ysb)) then
                        SellLic(2)
                    end imgui.SameLine()
                    if imgui.Button(' '..fa.ICON_LONG_ARROW_LEFT..u8'\nДве\n '..fa.ICON_LONG_ARROW_LEFT, imgui.ImVec2(35, Ysb*2+5)) then SellLic(3) end
                    imgui.SetCursorPos(imgui.ImVec2(Xleft2,193))
                    if imgui.Button(u8'Лицензия на водный транспорт', imgui.ImVec2(Xsb-40,Ysb)) then
                        SellLic(1)
                    end imgui.SetCursorPosX(Xleft2)
    ...
    if Window['Latest_Update'].v then
        ...
        imgui.Begin(u8'License Center Assistant: Последнии изменения##4', Window['Latest_Update'], imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
            imgui.BeginChild('Latest Update', imgui.ImVec2(609,500), true)
                ...
                imgui.Separator()
                imgui.TextRGB('{58ACFA}21.10.2020 (1.12)\n{FF8000}[1]{FFFFFF} Фикс бага с файлом настроек.', 2)
                imgui.Separator()
                imgui.TextRGB('{58ACFA}04.11.2020 (1.13)\n{FF8000}[1]{FFFFFF} Исравления в орфографии и грамматике.\n{FF8000}[2]{FFFFFF} Добавлена возможность продать сразу 2 лицензии.\n{FF8000}[3]{FFFFFF} Исправлен краш игры в функциях принятия в организацию.\n{FF8000}[4]{FFFFFF} Добавлена возможность включить ID-карту, вместо паспорта.\n{FF8000}[5]{FFFFFF} Переделана система рангов.\n• Так же при повышении, ранг меняется автоматически.\n{FF8000}[6]{FFFFFF} Убрана дата и время в докладе, после завершения экзамена.\n{FF8000}[7]{FFFFFF} Добавлена "Темно-синяя" тема.\n{FF8000}[8]{FFFFFF} Доработка биндера.', 2)
                imgui.Separator()
            imgui.EndChild()
        imgui.End()
    end
    ...
    if Window['FAQ'].v then
        ...
        imgui.Begin(u8'License Center Assistant: F.A.Q для новичков##4', Window['FAQ'], imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoScrollWithMouse)
            imgui.BeginChild('Latest Update', imgui.ImVec2(609,500), true)
                ...
                if imgui.CollapsingHeader(u8'Как начать и принимать экзамен.') then
                    local xPos = 25
                    imgui.TextRGB('Для того чтобы начать принимать экзамен с человеком, вы должны п')
                ...
                for str in u8:decode(Binder[i].text):gmatch('[^\n\r]+') do
                    if string.find(str, '^@wait(%d+)') then
                        ...
                        if string.find(str, '(.*)@nsent') then
                            ...
                            if string.find(str, '@local:(.*)') then
                                local strLocal = string.match(str, '@local:(.*)')
                                if string.find(strLocal, '{my_id}') then
                                    local str2 = string.gsub(strLocal, '{my_id}', myid)
                                    if string.find(str2, '{my_nick}') then
                                        local str3 = string.gsub(str2, '{my_nick}', rpnick)
                                        sampAddChatMessage(str3, -1)
                                    else
                                        sampAddChatMessage(str2, -1)
                                    end
                                elseif string.find(strLocal, '{my_nick}') then
                                    local strMyNick = string.gsub(strLocal, '{my_nick}', rpnick)
                                    sampAddChatMessage(strMyNick, -1)
                                else
                                    sampAddChatMessage(strLocal, -1)
                                end
                            elseif string.find(str, '{my_id}') then
			...
			lua_thread.create(function()
                for str in u8:decode(Binder[k].text):gmatch('[^\n\r]+') do
                    if string.find(str, '^@wait(%d+)') then
                        ...
                        if string.find(str, '(.*)@nsent') then
                            local key_down = table.concat(Binder[k].v)
                            if key_down == '1896' or key_down == '1897' or key_down == '1898' or key_down == '1899' or key_down == '18100' or key_down == '18101' or key_down == '18102' or key_down == '18103' or key_down == '18104' or key_down == '18105' then
                            alt_num = true else alt_num = false end
                            local strNsent = string.match(str, '(.*)@nsent')
                            if string.find(strNsent, '{my_id}') then
                                local str2 = string.gsub(strNsent, '{my_id}', myid)
                                if string.find(str2, '{my_nick}') then
                                    ...
                                    sampSetChatInputEnabled(true)
                                    sampSetChatInputText(str3)
                                    if alt_num then wait(10) setVirtualKeyDown(0x08, true) wait(20) setVirtualKeyDown(0x08, false) end
                                    repeat wait(0) until isKeyJustPressed(0x0D) or isKeyJustPressed(0x75) or isKeyJustPressed(0x1B)
                                else
                                    sampSetChatInputEnabled(true)
                                    sampSetChatInputText(str2)
                            ...
                            elseif string.find(strNsent, '{my_nick}') then
                                local strMyNick = string.gsub(strNsent, '{my_nick}', rpnick)
                                sampSetChatInputEnabled(true)
                                sampSetChatInputText(strMyNick)
                                if alt_num then wait(10) setVirtualKeyDown(0x08, true) wait(20) setVirtualKeyDown(0x08, false) end
                                repeat wait(0) until isKeyJustPressed(0x0D) or isKeyJustPressed(0x75) or isKeyJustPressed(0x1B)
                            else
                                sampSetChatInputEnabled(true)
...
function carExam()
    lua_thread.create(function()
        RunCarExamSt = true
        sampSendChat('Здравствуйте, меня зовут '..rpnick..', я буду принимать у Вас экзамен.')
        wait(2200)
        sampSendChat('Предъявите Ваш'..IDy..' '..Pass_or_id..IDy..'.')
        wait(2200)
        ...
        if isKeyJustPressed(0x31) then
            sampSendChat('/me достал'..Sex..' ручку и талон с папки')
            wait(2200)
            sampSendChat('/me вписал'..Sex..' имя и фамилию экзаменуемого, после чего передал'..Sex..' человеку напротив')
            wait(2200)
            sampSendChat('Пройдёмте за мной на парковку, для сдачи практической части экзамена.')
            if Toggle['DokladF'].v then
                wait(2200)
                if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                    sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Начал'..Sex..' принимать экзамен на легковое авто у '..targrpnick..'.')
                else
                    sampSendChat('/f Начал'..Sex..' принимать экзамен на легковое авто у '..targrpnick..'.')
                end
            end
            wait(1000)
            sampAddChatMessage(tag_text..' Для продолжения нажмите - {58ACFA}1{FFFFFF}. Отмена - {58ACFA}2{FFFFFF}.', tag_clr)
                ...
                if isKeyJustPressed(0x31) then
                    ...
                    wait(2200)
                    sampSendChat('Заводите двигатель, включайте фары и двигайтесь к началу полосы справа перед Вами.')
                    ...
                    RunCarExam = true
                    NextStepCar = 1
                    repeat wait(0) until NextStepCar == 7
                        if NextStepCar == 7 then
                            wait(1500)
                            ...
                            repeat wait(0) until isKeyJustPressed(0x31) or (isKeyJustPressed(0x32) and not isKeyDown(0x12)) or isKeyJustPressed(0x33)
                            if isKeyJustPressed(0x31) then
                                sampSendChat('/me приоткрыл'..Sex..' папку, после чего достал'..Sex..' бумаги и начал'..Sex..' заполнять их')
                                wait(2200)
                                ...
                                wait(2200)
                                sampSendChat('/me после заполнения документов, аккуратно сложил'..Sex..' их и закрыл'..Sex..' папку')
                                wait(2200)
                                if Toggle['DokladF'].v then
                                    if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                                        sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на легковое авто.')
                                    else
                                        sampSendChat('/f Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на легковое авто.')
                                    end
                                end
                                ExamEnd(1)
                                wait(2200)
...
function airExam()
    lua_thread.create(function()
        ...
        repeat wait(0) until isKeyJustPressed(0x31) or (isKeyJustPressed(0x32) and not isKeyDown(0x12)) or isKeyJustPressed(0x33)
        if isKeyJustPressed(0x31) then
            sampSendChat('/me достал'..Sex..' ручку и талон из папки')
            wait(2200)
            sampSendChat('/me вписал'..Sex..' имя и фамилию экзаменуемого, после чего передал'..Sex..' человеку напротив')
            wait(2200)
            ...
            if Toggle['DokladF'].v then
                wait(2200)
                if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                    sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Начал'..Sex..' принимать экзамен на воздушный транспорт с '..targrpnick..'.')
                else
                    sampSendChat('/f Начал'..Sex..' принимать экзамен на воздушный транспорт с '..targrpnick..'.') 
                end
            end
            wait(1000)
            sampAddChatMessage(tag_text..' Для продолжения нажмите - {58ACFA}1{FFFFFF}. Отмена - {58ACFA}2{FFFFFF}.', tag_clr)
                ...
                if isKeyJustPressed(0x31) then
                    ...
                    if isKeyJustPressed(0x31) then
                        ...
                        if Toggle['DokladF'].v then
                            if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                                sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на воздушный транспорт.')
                            else
                                sampSendChat('/f Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на воздушный транспорт.')
                            end
                        end
                        ExamEnd(1)
                        wait(2200)
    ...
    RunMotoExamSt = true
        ...
        if isKeyJustPressed(0x31) then
            ...
            if Toggle['DokladF'].v then
                wait(2200)
                if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                    sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Начал'..Sex..' принимать экзамен на мототранспорт с '..targrpnick..'.')
                else
                    sampSendChat('/f Начал'..Sex..' принимать экзамен на мототранспорт с '..targrpnick..'.')  
                end
            end
            wait(1000)
            sampAddChatMessage(tag_text..' Для продолжения нажмите - {58ACFA}1{FFFFFF}. Отмена - {58ACFA}2{FFFFFF}.', tag_clr)
                ...
                if isKeyJustPressed(0x31) then
                    ...
                    wait(2200)
                    sampSendChat('Заводите двигатель и двигайтесь к началу полосы слева от Вас.')
                    if Toggle['Checkpoints_st'].v then setMarker(2, -2071.9351, -172.0957, 35.3203, 3, 0xFF0000FF) end
                    wait(2200)
                    sampSendChat('/n Завести двигатель можно нажав на клавишу [Ctrl], включить фары [Alt].')
                    wait(1000)
                    sampAddChatMessage(tag_text..' Если экзаменуемый допустил {58ACFA}слишком много ошибок{FFFFFF} введите команду - {58ACFA}/fail', tag_clr)
                    RunMotoExam = true
                    NextStepMoto = 1
                    repeat wait(0) until NextStepMoto == 4
                        if NextStepMoto == 4 then
                            wait(1500)
                            sampAddChatMessage(tag_text..' После того как припарковались, нажмите:', tag_clr) wait(1)
                            sampAddChatMessage(tag_text..' Сдал - {58ACFA}1{FFFFFF}, Не сдал - {58ACFA}2{FFFFFF}, Отмена - {58ACFA}3{FFFFFF}.', tag_clr)
                            repeat wait(0) until isKeyJustPressed(0x31) or (isKeyJustPressed(0x32) and not isKeyDown(0x12)) or isKeyJustPressed(0x33)
                            if isKeyJustPressed(0x31) then
                                ...
                                if Toggle['DokladF'].v then
                                    if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                                        sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на мототранспорт.')
                                    else
                                        sampSendChat('/f Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на мототранспорт.')
                                    end
                                end
                                ExamEnd(1)
                                wait(2200)
...
function bigcarExam()
    ...
    RunBigCarExamSt = true
        ...
        if isKeyJustPressed(0x31) then
            sampSendChat('/me достал'..Sex..' ручку и талон из папки')
            wait(2200)
            sampSendChat('/me вписал'..Sex..' имя и фамилию экзаменуемого, после чего передал'..Sex..' человеку на против')
            wait(2200)
            sampSendChat('Пройдёмте за мной для сдачи практической части экзамена.')
            if Toggle['DokladF'].v then
                wait(2200)
                if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                    sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Начал'..Sex..' принимать экзамен на грузовой транспорт с '..targrpnick..'.')
                else
                    sampSendChat('/f Начал'..Sex..' принимать экзамен на грузовой транспорт с '..targrpnick..'.') 
                end
            end
            wait(1000)
            sampAddChatMessage(tag_text..' Для продолжения нажмите - {58ACFA}1{FFFFFF}. Отмена - {58ACFA}2{FFFFFF}.', tag_clr)
                        ...
                        if NextStepBigCar == 2 then
                            ...
                            if isKeyJustPressed(0x31) then
                                ...
                                if isKeyJustPressed(0x31) then
                                    ...
                                    if Toggle['DokladF'].v then
                                        if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                                            sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на грузовой транспорт.')
                                        else
                                            sampSendChat('/f Экзаменуемый №'..tid..' '..targrpnick..' успешно завершил экзамен на грузовой транспорт.')
                                        end
                                    end
                                    ExamEnd(1)
                                    wait(2200)
                                ...
                                elseif (isKeyJustPressed(0x32) and not isKeyDown(0x12)) then
                                    ...
                                    if Toggle['DokladF'].v then
                                        if Toggle['TagF'].v then -- '..u8:decode(Input['Tag'].v)..'
                                            sampSendChat('/f '..u8:decode(Input['Tag'].v)..' Экзаменуемый №'..tid..' '..targrpnick..' не сдал экзамен на грузовой транспорт.')
                                        else
                                            sampSendChat('/f Экзаменуемый №'..tid..' '..targrpnick..' не сдал экзамен на грузовой транспорт.') 
                                        end
                                    end
                                    ExamEnd(2)
                                    wait(2200)
    ...
    lua_thread.create(function()
        ...
        elseif isKeyJustPressed(0x32) and not isKeyDown(0x12) then
            ...
            if isKeyJustPressed(0x31) then
                ...
                thisScript():reload()
            elseif isKeyJustPressed(0x32) and not isKeyDown(0x12) then
                sampSendChat('У вас опечатка в '..Pass_or_id..IDe..PassE..', нужно поправить.')
                wait(2200)
                sampSendChat('/n У Вас нонРП ник, менять можно командой "/mn >> 7. Смена НонРП ника".')
                if SecondsExam > 0 then sampAddChatMessage(tag_text..' Как только пройдет 30 сек после начала, экзамен будет провален.', tag_clr) end
                repeat wait(0) until SecondsExam == 0 or SecondsExam < 0
                ExamEnd(2)
                wait(2200)
...
function SellLic(type)
    lua_thread.create(function()
        sampSendChat('Здравствуйте, меня зовут '..rpnick..'.')
        ...
        elseif type == 3 then
            if Krit['GunPriceSend'].v and not Krit['WaterPriceSend'].v then
                local GunPice = Round(Krit['GunPrice'].v)
                wait(2200) sampSendChat('Цена лицензии на оружие - '..GunPice..'$.')
            elseif not Krit['GunPriceSend'].v and Krit['WaterPriceSend'].v then
                local WaterPice = Round(Krit['WaterPrice'].v)
                wait(2200) sampSendChat('Цена лицензии на водный транспорт - '..WaterPice..'$.')
            elseif Krit['GunPriceSend'].v and Krit['WaterPriceSend'].v then
                local WaterPice = Round(Krit['WaterPrice'].v)
                local GunPice = Round(Krit['GunPrice'].v)
                wait(2200) sampSendChat('Цена лицензии на оружие - '..GunPice..'$.')
                wait(2200) sampSendChat('Цена лицензии на водный транспорт - '..WaterPice..'$.')
        ...
        wait(2200)
        sampSendChat('Предъявите мне Ваш'..IDy..' '..Pass_or_id..IDy..' и мед. карту.')
        wait(2200)
        ...
        if isKeyJustPressed(0x31) then
            ...
            wait(2200)
            sampSendChat('/me открыл'..Sex..' папку и вытащил'..Sex..' из него нужную лицензию')
            wait(2200)
            ...
            wait(2200)
            if type == 3 then
                sampSendChat('/selllic '..tid..' 1')
                repeat wait(0) until SellState
                    if SellState then
                        SellState = false
                        sampSendChat('/selllic '..tid..' 2')
                    end
            else
...
function sellinsurance()
    lua_thread.create(function()
        ...
        repeat wait(0) until isKeyJustPressed(0x31) or (isKeyJustPressed(0x32) and not isKeyDown(0x12))
        if isKeyJustPressed(0x31) then
            sampSendChat('Хорошо, предоставьте мне Ваш'..IDy..' '..Pass_or_id..IDy..', лицензии и ПТС.') 
            wait(2200)
            sampSendChat('/n Введите команду: /pass '..myid..', Лицензии: /lic '..myid,' Лицензии: /pts '..myid)
            wait(1000)
            ...
            elseif not Krit['NeedAgeInsur'].v and Krit['NeedMedInsur'].v and Krit['NeedLvlInsur'].v then
                ...
                repeat wait(0) until isKeyJustPressed(0x31) or (isKeyJustPressed(0x32) and not isKeyDown(0x12)) or isKeyJustPressed(0x33)
                if isKeyJustPressed(0x31) then
                    sampSendChat('/do Папка с бланками и документами, талонами и страховкой в правой руке.')
                    wait(2200)
                    sampSendChat('/me открыл'..Sex..' папку, достал'..Sex..' документы, после чего начал'..Sex..' заполнять их')
                    wait(2200)
                    sampSendChat('/me ниже поставил'..Sex..' печать "License center", дату и подпись')
                    wait(2200)
                    sampSendChat('/me после продажи страховки, всё аккуратно сложил'..Sex..' папку')
                    wait(2200)
    ...
    lua_thread.create(function()
        if st == 1 then
            ...
            if isKeyJustPressed(0x31) then
                ...
                sampSendChat('/me выбрал'..Sex..' сотрудника, затем нажал'..Sex..' кнопку "Повышение в должности"')
                wait(200)
                sampAddChatMessage(tag_text..' Впишите {58ACFA}id{FFFFFF} игрока', tag_clr) wait(100)
...
function sampGetPlayerIdByNickname(nick)
    ...
    if tostring(nick) == sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) then return select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)) end
...
function theme18()
    ...
    local ImVec4 = imgui.ImVec4
        ...
        colors[clr.TitleBg]                = ImVec4(0.00, 0.52, 0.74, 1.00);
        colors[clr.TitleBgActive]          = ImVec4(0.00, 0.52, 0.74, 1.00);
        colors[clr.CheckMark]              = ImVec4(0.13, 0.65, 0.87, 1.00);
        colors[clr.SliderGrab]             = ImVec4(0.00, 0.52, 0.74, 1.00);
        colors[clr.SliderGrabActive]       = ImVec4(0.00, 0.52, 0.74, 1.00);
        colors[clr.ButtonHovered]          = ImVec4(1.00, 0.39, 0.39, 1.00);
        colors[clr.Header]                 = ImVec4(0.00, 0.52, 0.74, 0.60);
        colors[clr.HeaderHovered]          = ImVec4(0.00, 0.52, 0.74, 0.43);
        colors[clr.ScrollbarGrabHovered]   = ImVec4(0.18, 0.22, 0.25, 1.00);
    colors[clr.Text]                   = ImVec4(0.95, 0.96, 0.98, 1.00);
    colors[clr.TextDisabled]           = ImVec4(0.29, 0.29, 0.29, 1.00);
    colors[clr.WindowBg]               = ImVec4(0.14, 0.14, 0.14, 1.00);
    colors[clr.ChildWindowBg]          = ImVec4(0.12, 0.12, 0.12, 1.00);
    colors[clr.PopupBg]                = ImVec4(0.08, 0.08, 0.08, 0.94);
    colors[clr.Border]                 = ImVec4(0.14, 0.14, 0.14, 0.40);
    colors[clr.BorderShadow]           = ImVec4(1.00, 1.00, 1.00, 0.10);
    colors[clr.FrameBg]                = ImVec4(0.20, 0.20, 0.20, 1.00);
    colors[clr.FrameBgHovered]         = ImVec4(0.00, 0.52, 0.74, 0.40);
    colors[clr.FrameBgActive]          = ImVec4(0.00, 0.52, 0.74, 0.90);
    colors[clr.TitleBgCollapsed]       = ImVec4(0.00, 0.52, 0.74, 0.79);
    colors[clr.MenuBarBg]              = ImVec4(0.20, 0.20, 0.20, 1.00);
    colors[clr.ScrollbarBg]            = ImVec4(0.02, 0.02, 0.02, 0.39);
    colors[clr.ScrollbarGrab]          = ImVec4(0.00, 0.52, 0.74, 0.80);
    colors[clr.ScrollbarGrabActive]    = ImVec4(0.24, 0.24, 0.24, 1.00);
    colors[clr.ComboBg]                = ImVec4(0.24, 0.24, 0.24, 1.00);
    colors[clr.Button]                 = ImVec4(0.00, 0.52, 0.74, 0.80);
    colors[clr.ButtonHovered]          = ImVec4(0.00, 0.52, 0.74, 0.63);
    colors[clr.ButtonActive]           = ImVec4(0.00, 0.52, 0.74, 1.00);
    colors[clr.HeaderActive]           = ImVec4(0.00, 0.52, 0.74, 0.80);
    colors[clr.ResizeGrip]             = ImVec4(0.00, 0.52, 0.74, 0.80);
    colors[clr.ResizeGripHovered]      = ImVec4(0.00, 0.52, 0.74, 0.63);
    colors[clr.ResizeGripActive]       = ImVec4(0.00, 0.52, 0.74, 1.00);
    colors[clr.CloseButton]            = ImVec4(0.40, 0.39, 0.38, 0.16);
    colors[clr.CloseButtonHovered]     = ImVec4(0.40, 0.39, 0.38, 0.39);
    colors[clr.CloseButtonActive]      = ImVec4(0.40, 0.39, 0.38, 1.00);
    colors[clr.PlotLines]              = ImVec4(0.61, 0.61, 0.61, 1.00);
    colors[clr.PlotLinesHovered]       = ImVec4(1.00, 0.43, 0.35, 1.00);
    colors[clr.PlotHistogram]          = ImVec4(0.00, 0.52, 0.74, 1.00);
    colors[clr.PlotHistogramHovered]   = ImVec4(1.00, 0.18, 0.18, 1.00);
    colors[clr.TextSelectedBg]         = ImVec4(1.00, 0.32, 0.32, 1.00);
    colors[clr.ModalWindowDarkening]   = ImVec4(0.26, 0.26, 0.26, 0.60);
end
...
function setstyleandtheme()
    ...
    if Theme.v == 18 then theme18() end
end